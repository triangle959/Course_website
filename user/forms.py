...
def nickname_validate(nickname):
    user_obj = UserProfile.objects.filter(nickname=nickname).first()
    if user_obj:
        raise ValidationError('此昵称已经存在，请换一个')
...
class SignupForm(forms.Form):
    ...
    password = forms.CharField(min_length=6, max_length=50, required=True,
                               error_messages={'required': '密码不能为空',
                                               ...
                                               'min_length': '密码不能少于6位',
                                               'max_length': '密码最多50位'})
    #classinfo = forms.ChoiceField(required=True, error_messages={'required': '班级不能为空'})
    #classInfo = forms.CharField(max_length=10, widget=forms.widgets.Select(choices=classInfo_list))
    nickname = forms.CharField(validators=[nickname_validate,], required=True,
                               error_messages={'required': '昵称不能为空'})
    classInfo = forms.ModelChoiceField(queryset=ClassInfo.objects.all(), required=True, empty_label=None, initial="15医药软件")
