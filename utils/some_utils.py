...
from Course_website.settings import AVATAR_FILE_PATH,UPLOAD_FILE_PATH
...
def save_upload_file(file):
    ...
    file_type = file.name.split('.')[-1]
    save_file_name = random_code + '.' + file_type
    file_path = os.path.join(UPLOAD_FILE_PATH, save_file_name)
    # print(file_path)
    with open(file_path, 'wb') as f:
        for chunk in file.chunks():
            f.write(chunk)
    for_download_path = file_path.replace('\\', '/')
    return for_download_path
